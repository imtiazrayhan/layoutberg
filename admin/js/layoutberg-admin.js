...
(function($) {
    ...
    window.LayoutBergAdmin = {
        ...
        bindEvents: function() {
            ...
            $(document).on('click', '.layoutberg-modal-backdrop', this.closeModal.bind(this));
            
            // Generation actions
            $(document).on('click', '#layoutberg-generate', this.generateLayout.bind(this));
            $(document).on('click', '.layoutberg-regenerate', this.regenerateLayout.bind(this));
            $(document).on('click', '.layoutberg-apply-layout', this.applyLayout.bind(this));
            
            // Template actions
            $(document).on('click', '#layoutberg-save-template', this.saveTemplate.bind(this));
            $(document).on('click', '[data-template]', this.quickTemplate.bind(this));
            
            // Interactive elements
            $(document).on('click', '.layoutberg-btn', this.handleButtonClick.bind(this));
            $(document).on('input', '.layoutberg-input, .layoutberg-textarea', this.handleInputChange.bind(this));
            $(document).on('change', '.layoutberg-select', this.handleSelectChange.bind(this));
            
            // Dashboard interactions
            $(document).on('click', '#layoutberg-dismiss-guide', this.dismissGuide.bind(this));
            $(document).on('click', '.layoutberg-refresh-stats', this.refreshStats.bind(this));
            
            // Copy to clipboard
            $(document).on('click', '[data-copy]', this.copyToClipboard.bind(this));
            
            // Form validation
            $(document).on('submit', 'form', this.validateForm.bind(this));
            
            // Responsive menu toggle
            $(document).on('click', '.layoutberg-menu-toggle', this.toggleMobileMenu.bind(this));
            
            // Search functionality
            $(document).on('input', '.layoutberg-search', this.performSearch.bind(this));
        ...
        initTooltips: function() {
            ...
            $('.layoutberg-help-tip, [data-tooltip]').each(function() {
                var $this = $(this);
                var content = $this.data('tooltip') || $this.attr('title') || $this.data('tip');
        ...
        showTooltip: function(element, content) {
            ...
            var $tooltip = $('<div class="layoutberg-tooltip">' + content + '</div>');
            $('body').append($tooltip);
        ...
        initKeyboardShortcuts: function() {
            $(document).on('keydown', function(e) {
                ...
                if ((e.ctrlKey || e.metaKey) && e.key === 's') {
                    e.preventDefault();
                    var $saveBtn = $('.layoutberg-btn[type="submit"]').first();
                    if ($saveBtn.length) {
                        $saveBtn.click();
                        this.showToast('Settings saved with keyboard shortcut!', 'info');
                    }
                }
        ...
        initThemeHandling: function() {
            ...
            $(document).on('click', '.layoutberg-theme-toggle', function() {
                ...
                localStorage.setItem('layoutberg-theme', isDark ? 'dark' : 'light');
            });
        ...
        initCharts: function() {
            ...
            $('.layoutberg-chart').each(function() {
                var $chart = $(this);
        ...
        renderChart: function($container, data) {
            ...
            data.forEach(function(item) {
                ...
                html += '<div class="layoutberg-chart-bar" style="height: ' + height + '%;" data-value="' + item.value + '">';
                html += '<div class="layoutberg-chart-bar-label">' + item.label + '</div>';
                html += '</div>';
            });
        ...
        initProgressBars: function() {
            $('.layoutberg-progress-bar').each(function() {
                ...
                $bar.find('.layoutberg-progress-fill').animate({ width: progress + '%' }, 1000);
            });
        },
                ...
                $(document).on('heartbeat-tick', function(e, data) {
                    if (data.layoutberg_heartbeat) {
                        ...
                        if (data.layoutberg_heartbeat.notifications) {
                            this.handleNotifications(data.layoutberg_heartbeat.notifications);
                        }
                        if (data.layoutberg_heartbeat.stats) {
                            this.updateStats(data.layoutberg_heartbeat.stats);
                        }
                    }
                }.bind(this));
            }
        ...
        showLoading: function(element, text) {
            ...
            $element
                ...
                .prop('disabled', true)
                .html('<span class="layoutberg-spinner"></span> ' + (text || 'Loading...'));
        },
        ...
        hideLoading: function(element, text) {
            var $element = $(element);
            var originalHtml = $element.data('original-html') || text || 'Button';
        ...
        showToast: function(message, type, duration) {
            ...
            var $toast = $('<div class="layoutberg-toast layoutberg-toast-' + type + '">' + message + '</div>');
            
            // Add to container
            var $container = $('.layoutberg-toast-container');
            if (!$container.length) {
                $container = $('<div class="layoutberg-toast-container"></div>');
        ...
        showNotice: function(message, type, persistent) {
            ...
            var $notice = $('<div class="layoutberg-alert layoutberg-alert-' + type + ' layoutberg-fade-in">');
            $notice.html('<span class="dashicons dashicons-' + this.getNoticeIcon(type) + '"></span><div><strong>' + message + '</strong></div>');
            
            if (!persistent) {
                $notice.append('<button class="layoutberg-alert-dismiss" aria-label="Dismiss">&times;</button>');
            }
        ...
        copyToClipboard: function(e) {
            e.preventDefault();
            var $button = $(e.currentTarget);
            var text = $button.data('copy') || $button.closest('.copy-container').find('input, textarea').val();
        ...
        initDragAndDrop: function() {
            // File upload areas
            $('.layoutberg-upload-area').on('dragover dragenter', function(e) {
                e.preventDefault();
        ...
        handleFileUpload: function(files, $container) {
            for (var i = 0; i < files.length; i++) {
                ...
                // Validate file type
                if (!this.isValidFileType(file, $container.data('accept'))) {
                    this.showNotice('Invalid file type: ' + file.name, 'error');
                    continue;
                }
        ...
        generateLayout: function(e) {
            ...
            if (!prompt) {
                this.showNotice('Please enter a prompt to generate a layout', 'error');
                return;
            }
            ...
            setTimeout(function() {
                ...
                this.showNotice('Layout generated successfully!', 'success');
            }.bind(this), 3000);
        },
    ...
    $(window).on('resize', function() {
        ...
        $('.layoutberg-admin-page').toggleClass('mobile', $(window).width() < 768);
    });
