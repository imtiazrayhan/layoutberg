 ...
 */
const LayoutBergSidebar = ( {
	onGenerate,
	onSaveAsTemplate,
	settings,
	onSettingsChange,
} ) => {
	const { selectedBlocks, hasSelectedBlocks, totalBlocks } = useSelect(
		( select ) => {
		...
		<Fragment>
			...
			<Card className="layoutberg-sidebar-card">
				<CardHeader>
					<Flex>
						<FlexItem>
							<strong>
								{ __( 'AI Layout Generator', 'layoutberg' ) }
							</strong>
						</FlexItem>
						<FlexItem>{ layout }</FlexItem>
					</Flex>
				</CardHeader>
				<CardBody>
					<p className="layoutberg-sidebar-description">
						{ hasSelectedBlocks
							? __(
									'Replace selected blocks with AI-generated layout',
									'layoutberg'
							  )
							: __(
									'Generate a new layout with AI assistance',
									'layoutberg'
							  ) }
					</p>
			...
			<Card className="layoutberg-sidebar-card">
				...
				<CardBody>
					<p className="layoutberg-sidebar-description">
						{ __(
							'Save your current layout as a reusable template',
							'layoutberg'
						) }
					</p>
				...
				>
					<div className="layoutberg-tips">
						<div className="layoutberg-tip">
							<strong>
								{ __( 'Keyboard Shortcut:', 'layoutberg' ) }
							</strong>
							<p>
								{ __(
									'Press Ctrl+Shift+L (or Cmd+Shift+L on Mac) to quickly open the generation modal.',
									'layoutberg'
								) }
							</p>
						</div>
						...
						<div className="layoutberg-tip">
							<strong>
								{ __( 'Better Prompts:', 'layoutberg' ) }
							</strong>
							<p>
								{ __(
									'Be specific about your needs: "Create a hero section for a tech startup with a call-to-action button"',
									'layoutberg'
								) }
							</p>
						</div>
						...
						<div className="layoutberg-tip">
							<strong>
								{ __( 'Replace vs Insert:', 'layoutberg' ) }
							</strong>
							<p>
								{ __(
									'Select blocks to replace them, or place cursor where you want to insert new content.',
									'layoutberg'
								) }
							</p>
						</div>
				...
				>
					<PanelRow>
						<Flex direction="column" gap={ 2 }>
							<FlexItem>
								<strong>
									{ __( 'Total Blocks:', 'layoutberg' ) }{ ' ' }
								</strong>
								{ totalBlocks }
							</FlexItem>
							<FlexItem>
								<strong>
									{ __( 'Selected:', 'layoutberg' ) }{ ' ' }
								</strong>
								{ hasSelectedBlocks
									? selectedBlocks.length
									: 0 }
							...
							<FlexItem>
								<strong>
									{ __( 'Next Action:', 'layoutberg' ) }{ ' ' }
								</strong>
								{ hasSelectedBlocks
									? __(
											'Will replace selected blocks',
											'layoutberg'
									  )
									: __(
											'Will insert at current position',
											'layoutberg'
									  ) }
							</FlexItem>
						</Flex>
