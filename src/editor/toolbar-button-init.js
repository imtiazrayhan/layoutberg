...
const addLayoutBergButton = () => {
    ...
    const insertButton = () => {
        ...
        if (buttonAdded && document.getElementById('layoutberg-toolbar-button')) {
            return true;
        }
        ...
        for (const selector of selectors) {
            ...
            if (targetElement) {
                console.log('LayoutBerg: Found toolbar element with selector:', selector);
                break;
            }
        }
        ...
        const existingButton = document.getElementById('layoutberg-toolbar-button');
        if (existingButton) {
        ...
        buttonContainer.style.cssText = 'display: inline-flex; align-items: center; margin: 0 8px;';
        
        // Create the button
        const button = document.createElement('button');
        button.className = 'components-button layoutberg-top-toolbar-button has-icon has-text';
        button.type = 'button';
        ...
        textSpan.style.cssText = 'display: inline-block !important; margin-left: 4px; font-size: 13px; font-weight: 600;';
        ...
        button.addEventListener('click', (e) => {
            ...
            if (window.layoutbergOpenModal) {
                ...
                const sidebarButton = document.querySelector('[aria-label="LayoutBerg"]');
                if (sidebarButton) {
                    sidebarButton.click();
                    setTimeout(() => {
                        const generateButton = document.querySelector('.layoutberg-generate-button');
                        if (generateButton) {
        ...
        const insertInRightToolbar = () => {
            ...
            if (targetElement.classList.contains('editor-header__settings')) {
        ...
        const updateButtonText = () => {
            ...
            try {
                ...
                const selectedBlockIds = blockEditorStore.getSelectedBlockClientIds();
    ...
    if (window.MutationObserver) {
        ...
        const editorElement = document.querySelector('.edit-post-layout, .editor-editor-interface');
        if (editorElement) {
