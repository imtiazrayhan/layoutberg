...
const LayoutBergEditor = () => {
	...
	const [ generationState, dispatch ] = useReducer(
		generationReducer,
		initialGenerationState
	);
	...
	const { selectedBlocks, hasSelectedBlocks, allBlocks } = useSelect(
		( select ) => {
	...
	const handleGenerate = async ( variationOptions = null ) => {
		...
		try {
			...
			dispatch( {
				type: GENERATION_ACTIONS.UPDATE_STATE,
				payload: 'sending',
			} );
			...
			if ( response.success && response.data && response.data.blocks ) {
				...
				if ( ! parsedBlocks || parsedBlocks.length === 0 ) {
					throw new Error(
						__(
							'No valid blocks found in generated content.',
							'layoutberg'
						)
					);
				}
				...
				if ( validBlocks.length === 0 ) {
					throw new Error(
						__(
							'Generated content contained only empty blocks.',
							'layoutberg'
						)
					);
				}
				...
				const { removeBlocks, insertBlocks: insertBlocksAction } =
				...
				if ( hasSelectedBlocks ) {
					...
					createNotice(
						'success',
						__(
							'Layout generated and replaced selected blocks!',
							'layoutberg'
						),
						{
							type: 'snackbar',
				...
				actions: [
					{
						...
						url:
							layoutbergEditor.templatesUrl ||
							'/wp-admin/admin.php?page=layoutberg-templates',
					},
				],
			}
		);
		...
		if ( urlParams.get( 'hide_pattern_modal' ) === '1' ) {
			...
			if ( dispatch( 'core/edit-post' ) ) {
				...
				const unsubscribe = wp.data.subscribe( () => {
					const isModalOpen = wp.data
						.select( 'core/edit-post' )
						?.isModalActive( 'core/edit-post/start-page-options' );
					if ( isModalOpen ) {
			...
			apiFetch( {
				...
				.then( ( response ) => {
					if ( response && response.content ) {
						...
						const parsedBlocks = wp.blocks.parse(
							response.content
						);
						if ( parsedBlocks && parsedBlocks.length > 0 ) {
							const { insertBlocks: insertBlocksAction } =
								wp.data.dispatch( 'core/block-editor' );
							...
							createNotice(
								...
								sprintf(
									__(
										'Template "%s" loaded successfully!',
										'layoutberg'
									),
									response.name
								),
		...
		<Fragment>
			...
			{ isSaveTemplateModalOpen && (
				<SaveTemplateModal
					...
					blocks={
						window.layoutbergCurrentBlocks ||
						generationState.lastGeneratedBlocks
					}
					prompt={ prompt }
				/>
			) }
