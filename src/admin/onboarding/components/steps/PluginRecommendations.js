...
import { Button, Card, CardBody, Notice, ExternalLink } from '@wordpress/components';
...
const PluginRecommendations = ({ onNext, isLoading }) => {
	...
	const [plugins, setPlugins] = useState(layoutbergOnboarding.plugins || {});
	...
	const installPlugin = async (slug) => {
		...
		try {
			...
			setInstallErrors({
				...installErrors,
				[slug]: error.message || __('Installation failed', 'layoutberg'),
			});
		}
	...
	const pluginCards = [
		{
			...
			description: __('A collection of essential blocks to supercharge your content creation. Includes advanced blocks like content toggle, testimonials, countdown, and more.', 'layoutberg'),
			features: [
		...
		},
		{
			...
			description: __('Create beautiful, responsive tables with advanced features like sorting, filtering, and custom styling - all within Gutenberg.', 'layoutberg'),
			features: [
	...
	return (
		<div className="layoutberg-onboarding__step layoutberg-onboarding__step--plugins">
			...
			<p className="layoutberg-onboarding__step-description">
				{__('These plugins work perfectly with LayoutBerg to give you even more design possibilities.', 'layoutberg')}
			</p>
			...
			<div className="layoutberg-onboarding__plugins-grid">
				{pluginCards.map((pluginInfo) => {
					...
					return (
						<Card key={pluginInfo.slug} className="layoutberg-onboarding__plugin-card">
							<CardBody>
								...
								<div className="layoutberg-onboarding__plugin-actions">
									{!isActive && (
										<Button
											variant="secondary"
											onClick={() => installPlugin(pluginInfo.slug)}
											isBusy={isInstalling}
											disabled={isInstalling}
										>
											{isInstalled
												? __('Activate', 'layoutberg')
												: __('Install & Activate', 'layoutberg')}
										</Button>
									)}
									{isActive && (
