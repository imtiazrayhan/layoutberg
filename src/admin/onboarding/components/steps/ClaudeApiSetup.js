...
import { Button, TextControl, Notice, ExternalLink } from '@wordpress/components';
...
const ClaudeApiSetup = ({ data, onNext, isLoading, onboardingData }) => {
	...
	const validateApiKey = async () => {
		...
		try {
			...
			setValidationError(
				error.message || __('Invalid API key. Please check and try again.', 'layoutberg')
			);
			setIsValid(false);
		}
	...
	return (
		<div className="layoutberg-onboarding__step layoutberg-onboarding__step--api-setup">
			...
			<p className="layoutberg-onboarding__step-description">
				{__('Add Claude for even more AI model options. Claude excels at understanding complex instructions and creating sophisticated layouts.', 'layoutberg')}
			</p>
			...
			{onboardingData?.has_claude_key && (
				<Notice
					...
					isDismissible={false}
				>
					{__('Great! You already have a Claude API key configured. You can update it below or continue to the next step.', 'layoutberg')}
				</Notice>
			)}

			<div className="layoutberg-onboarding__api-setup-content">
				<div className="layoutberg-onboarding__api-instructions">
					<h3>{__('How to get your Claude API key:', 'layoutberg')}</h3>
					<ol>
						<li>
							{__('Visit', 'layoutberg')}{' '}
						...
						<li>{__('Sign up or log in to your account', 'layoutberg')}</li>
						<li>
							{__('Go to', 'layoutberg')}{' '}
						...
						<li>{__('Copy the key and paste it below', 'layoutberg')}</li>
					</ol>
				</div>
				...
				<div className="layoutberg-onboarding__api-form">
					<TextControl
						...
						type="password"
						placeholder="sk-ant-..."
						help={__('Your API key is encrypted and stored securely', 'layoutberg')}
					...
					{isValid && !onboardingData?.has_claude_key && (
						<Notice
							...
							isDismissible={false}
						>
							{__('API key validated successfully!', 'layoutberg')}
						</Notice>
					)}
			...
			<div className="layoutberg-onboarding__api-info">
				...
				<ul>
					<li>{__('Superior understanding of complex layout requirements', 'layoutberg')}</li>
					<li>{__('Excellent at following detailed instructions', 'layoutberg')}</li>
					<li>{__('Creates more sophisticated and nuanced designs', 'layoutberg')}</li>
					<li>{__('Great for professional and enterprise projects', 'layoutberg')}</li>
				</ul>
			</div>
		</div>
	);
