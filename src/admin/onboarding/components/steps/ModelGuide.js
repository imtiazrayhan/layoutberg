...
const ModelGuide = ({ data, onNext, isLoading }) => {
	const [selectedModel, setSelectedModel] = useState(data.default_model || 'gpt-3.5-turbo');
	...
	const models = [
		// OpenAI Models
		{
			...
			description: __('Fast, affordable, and great for most layouts', 'layoutberg'),
			pros: [
			...
			cons: [
				__('Less creative for complex designs', 'layoutberg'),
			],
		...
		},
		{
			...
			description: __('Superior quality for complex, creative layouts', 'layoutberg'),
			pros: [
		...
		},
		{
			...
			description: __('GPT-4 quality with improved speed', 'layoutberg'),
			pros: [
			...
			cons: [
				__('More expensive than GPT-3.5', 'layoutberg'),
			],
		...
		// Claude Models
		{
			...
			description: __('Most advanced Claude model with excellent performance', 'layoutberg'),
			pros: [
			...
			cons: [
				__('Requires Claude API key', 'layoutberg'),
			],
		...
		},
		{
			...
			description: __('Top-tier model for the most demanding tasks', 'layoutberg'),
			pros: [
		...
		},
		{
			...
			description: __('Lightning-fast model for simple layouts', 'layoutberg'),
			pros: [
			...
			cons: [
				__('Less capable for complex designs', 'layoutberg'),
			],
	...
	return (
		<div className="layoutberg-onboarding__step layoutberg-onboarding__step--model-guide">
			...
			<p className="layoutberg-onboarding__step-description">
				{__('Different models offer different capabilities. You can always change this later.', 'layoutberg')}
			</p>
					...
					>
						<CardBody>
							<div className="layoutberg-onboarding__model-header">
								<div>
									<h3>{model.name}</h3>
									<span className="layoutberg-onboarding__model-provider">{model.provider}</span>
								</div>
								{model.badge && (
							...
							<div className="layoutberg-onboarding__model-meta">
								<div className="layoutberg-onboarding__model-pricing">
									<strong>{__('Cost:', 'layoutberg')}</strong> {model.pricing}
								</div>
								<div className="layoutberg-onboarding__model-speed">
									<strong>{__('Speed:', 'layoutberg')}</strong> {model.speed}
								</div>
							</div>
							...
							<div className="layoutberg-onboarding__model-select">
								...
								/>
								<span>{__('Select this model', 'layoutberg')}</span>
							</div>
						</CardBody>
					</Card>
				))}
			...
			<div className="layoutberg-onboarding__model-tip">
				...
				<p>
					{__('Tip: Start with GPT-3.5 Turbo for quick results, or try Claude 3.5 Sonnet for more sophisticated layouts. You can change models anytime in settings.', 'layoutberg')}
				</p>
			</div>
