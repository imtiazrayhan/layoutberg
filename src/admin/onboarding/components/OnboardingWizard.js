...
const OnboardingWizard = () => {
	...
	const handleComplete = async () => {
		...
		try {
			...
			window.location.href = response.redirect_url || layoutbergOnboarding.dashboardUrl;
		} catch (error) {
	...
	const handleSkip = async () => {
		if (window.confirm(__('Are you sure you want to skip the onboarding? You can always configure settings later.', 'layoutberg'))) {
			setIsLoading(true);
			try {
				const response = await apiFetch({
				...
				window.location.href = response.redirect_url || layoutbergOnboarding.settingsUrl;
			} catch (error) {
